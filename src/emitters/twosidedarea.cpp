...
class TwoSidedAreaLight final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_shape, m_medium)
    MI_IMPORT_TYPES(Scene, Shape, Texture)

    TwoSidedAreaLight(const Properties &props) : Base(props) {
    ...
    void traverse(TraversalCallback *callback) override {
        ...
        callback->put("radiance", m_radiance, ParamFlags::Differentiable);
    }
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        sample_dir.x() = dr::select(flip, dr::fmadd(sample_dir.x(), 2.f, -1.f),
                                   sample_dir.x() * 2.f);
        Vector3f local = warp::square_to_cosine_hemisphere(sample_dir);
        ...
        Spectrum weight = pos_weight * wav_weight * (2.f * dr::Pi<ScalarFloat>);
    ...
    sample_direction(const Interaction3f &it, const Point2f &sample, Mask active) const override {
        ...
        if constexpr (drjit::is_jit_v<Float>) {
            ...
            Assert(m_shape, "Can't sample from a two-sided area emitter without an "
                            "associated Shape.");
        }
        ...
        if (likely(!m_radiance->is_spatially_varying())) {
            ...
            si = m_shape->eval_parameterization(uv, +RayFlags::FollowShape | +RayFlags::Default, active);
    ...
    Float pdf_direction(const Interaction3f &it, const DirectionSample3f &ds,
        ...
        Float dp = dr::dot(ds.d, ds.n);
        Float abs_dp = dr::abs(dp);
        // Two-sided: only filter degeneracy, not sign
        active &= (abs_dp > 0.f);
        ...
        if constexpr (drjit::is_jit_v<Float>) {
            ...
            Assert(m_shape,
                   "The two-sided area emitter without has no associated Shape!");
        }
        ...
        if (!m_radiance->is_spatially_varying()) {
            ...
            value = m_radiance->pdf_position(ds.uv, active) * dr::square(ds.dist) /
                    (dr::norm(dr::cross(si.dp_du, si.dp_dv)) * abs_dp);
        }
    ...
    sample_position(Float time, const Point2f &sample,
        ...
        if (!m_radiance->is_spatially_varying()) {
            ...
            auto si = m_shape->eval_parameterization(uv, +RayFlags::FollowShape | +RayFlags::Default, active);
            active &= si.is_valid();
            pdf /= dr::norm(dr::cross(si.dp_du, si.dp_dv));
    ...
    std::string to_string() const override {
        ...
        oss << "TwoSidedAreaLight[" << std::endl
    ...
    MI_DECLARE_CLASS(TwoSidedAreaLight)
    MI_TRAVERSE_CB(Base, m_radiance)
private:
    ref<Texture> m_radiance;
};

MI_EXPORT_PLUGIN(TwoSidedAreaLight)
